...
---
Ce jeu de données contient 3273 individus et 25 variables, 1 variable qualitative est illustrative.
...
Les 2 premiers axes de l' analyse expriment **53.92%** de l'inertie totale du jeu de données ; cela signifie que 53.92% de la variabilité totale du nuage des individus (ou des variables) est représentée dans ce plan.
...
Cette valeur est nettement supérieure à la valeur référence de **9.67%**, la variabilité expliquée par ce plan est donc hautement significative
(cette intertie de référence est le quantile 0.95-quantile de la distribution des pourcentages d'inertie obtenue en simulant 697 jeux de données aléatoires de dimensions comparables sur la base d'une distribution normale).
...
Ces composantes révèlent un taux d'inertie supérieur à celle du quantile 0.95-quantile de distributions aléatoires (67.19% contre 18.96%).
